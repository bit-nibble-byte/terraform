...
func (s *State) uploadTo(ctx context.Context, url string, data []byte) error {
...
func (s *State) uploadState(lineage string, serial uint64, isForcePush bool, state, jsonState, jsonStateOutputs []byte) error {
	...
	g.Go(func() error {
		return s.uploadTo(ctx, sv.UploadURL, state)
	})
	g.Go(func() error {
		return s.uploadTo(ctx, sv.JSONUploadURL, jsonState)
	})
